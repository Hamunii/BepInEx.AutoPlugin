...
public sealed class AutoPluginGenerator : IIncrementalGenerator
{
    ...
    static PluginClass? ToPluginClass(GeneratorAttributeSyntaxContext ctx)
    {
        ...
        var id = autoAttribute.ConstructorArguments[0].Value as string;
        var name = autoAttribute.ConstructorArguments[1].Value as string;
        var version = autoAttribute.ConstructorArguments[2].Value as string;
    ...
    )
    {
        try
        {
            ...
            string id = SymbolDisplay.FormatLiteral(plugin.Overrides.Id ?? props.Id!, true);
            string name = SymbolDisplay.FormatLiteral(plugin.Overrides.Name ?? props.Name!, true);
            string version = SymbolDisplay.FormatLiteral(
                plugin.Overrides.Version ?? props.Version!,
                true
            );
    ...
    public record struct PluginClass(string? Namespace, string ClassName, PluginProps Overrides);
